...
Connection.prototype._sendCommand = function (data) {
  var json = JSON.parse(data);
  this._debug('send command: %s', data);
  this._protocol.sendCommand(json);
};
...
Connection.prototype.registerKey = function (key, ttl, callback) {
  ...
  this._protocol.sendCommand(['set', newKey, ttl], function (err) {
    callback(err, key);
  });
};
...
Connection.prototype.deleteKey = function (key, callback) {
  ...
  this._protocol.sendCommand(['del', newKey], function (err) {
    callback(err, key);
  });
};
...
Connection.prototype.deleteKeys = function (keys, callback) {
  ...
  this._protocol.sendCommand(args, function (err) {
    callback(err, keys);
  });
};
...
Connection.prototype.keys = function (pattern, callback) {
  ...
  this._protocol.sendCommand(['keys', newKey], callback);
};
